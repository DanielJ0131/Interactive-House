 ...
 * HomeScreen
 *
 * Main dashboard for the Interactive House application.
 * Displays all rooms and their connected devices in one unified view.
 *
 * This screen acts as a global control center where the user can:
 * - Toggle lights, doors and windows
 * - Start coffee machines
 * - Monitor brewing progress
 *
 * All device state changes are handled locally in component state.
 * No backend persistence is implemented at this stage.
 */
export default function HomeScreen() {

  // Local state holding all rooms and their device states
  const [rooms, setRooms] = useState(initialData.rooms);

  /**
   * Tracks brewing progress for coffee machines.
   * Each key represents a device ID with a progress value (0–100).
   */
  const [brewingStatus, setBrewingStatus] = useState<{ [key: string]: number }>({});

  /**
   * Returns correct icon name and color depending on:
   * - Device type
   * - Current state
   * - Brewing progress (for coffee machines)
   */
  ...
  /**
   * Starts brewing simulation for a coffee machine.
   *
   * - Increases progress by 10% every 300ms
   * - Stops at 100%
   * - Resets back to 0 after 2 seconds
   *
   * Prevents duplicate brewing processes from running.
   */
  ...
  /**
   * Toggles device state inside a specific room.
   *
   * - Lights and coffee machines use on/off
   * - Doors and windows use open/closed
   *
   * Updates nested room/device state immutably.
   */
        ...
        {rooms.map((room) => (
          <View key={room.id} className="mb-8">
            ...
            {room.devices.map((device) => {
              ...
              const isActive = isCoffee
                ? progress > 0
                : ['on', 'open'].includes(device.state);

              const icon = getDeviceIconDetails(
                device.type,
                device.state,
                device.id
              );
              ...
              return (
                <View
                  key={device.id}
                  className="bg-slate-900/40 border border-slate-800/60 p-6 rounded-3xl mb-3 shadow-sm"
                >
                  ...
                  <View className="flex-row justify-between items-center">
                    ...
                    <View className="flex-row items-center">
                      ...
                      <View>
                        ...
                        <Text
                          className={`capitalize font-bold mt-1 ${
                            isCoffee && progress > 0
                              ? 'text-purple-400'
                              : (isActive
                                  ? 'text-sky-400'
